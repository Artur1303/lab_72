...
async function like_quote(event) {
    ...
    // console.log(event.target.parentElement.getElementsByTagName('p')[1])
    let data = await makeRequest('http://localhost:8000/api/quote/'+event.target.id + '/ratingup/', 'POST').then(response => response.json())
    console.log(data)
    quote = await  makeRequest('http://localhost:8000/api/quote/'+event.target.id, 'GET').then(response => response.json())
    event.target.parentElement.getElementsByTagName('p')[1].innerText = 'Raiting' + ':' + quote['rating']
}

async function dislike_quote(event) {
    event.preventDefault()
    let data = await makeRequest('http://localhost:8000/api/quote/'+event.target.id + '/ratingdown/', 'DELETE').then(response => response.json())
    console.log(data)
    quote = await  makeRequest('http://localhost:8000/api/quote/'+event.target.id, 'GET').then(response => response.json())
    event.target.parentElement.getElementsByTagName('p')[1].innerText = 'Raiting' + ':' + quote['rating']
}
...
async function get_quote(){
    ...
    for (quote of data) {
        let div = document.createElement('div');
            ...
            p1.innerText = 'created' + ':' + quote['created_at']
...
window.addEventListener('load', function() {
    ...
    home_button.onclick =async function(event){
        event.preventDefault()
        await get_quote()
    }
